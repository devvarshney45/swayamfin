    ...
    <div className={`${isDark ? 'bg-[#020617] text-white' : 'bg-[#F8FAFC] text-slate-900'} min-h-screen pb-32 font-dmsans transition-colors duration-300`}>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center py-4 md:py-6">
            <div className="flex items-center gap-3 md:gap-5">
              ...
              <div>
                <h1 className={`text-lg md:text-2xl font-black ${isDark ? 'text-white' : 'text-slate-900'} tracking-tight leading-none mb-1 md:mb-2`}>
                   Agent Portal
                </h1>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-6 md:mt-12">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8 mb-8 md:mb-12">
           ...
           <div className={`lg:col-span-1 p-6 md:p-8 ${isDark ? 'bg-white/5 border-white/5 shadow-2xl' : 'bg-white border-slate-200 shadow-xl'} rounded-[32px] md:rounded-[40px] border flex flex-col justify-between relative overflow-hidden group`}>
              ...
              <div>
                ...
                <p className={`text-[10px] md:text-[11px] font-bold ${isDark ? 'text-slate-500' : 'text-slate-500'} uppercase tracking-widest text-center md:text-left`}>
           ...
           <div className="lg:col-span-2 flex flex-col sm:flex-row gap-4 items-center">
              <div className="relative flex-1 group w-full">
                ...
                <input 
                  ...
                  className={`w-full pl-12 pr-6 py-4 md:py-5 rounded-2xl md:rounded-[28px] ${isDark ? 'bg-white/5 border-white/10 text-white placeholder-slate-700' : 'bg-white border-slate-200 text-slate-900 shadow-sm'} focus:outline-none border-2 focus:border-blue-500 transition-all font-black text-xs md:text-sm shadow-inner`}
                  value={searchTerm}
        ...
        <div className="flex flex-col sm:flex-row gap-4 items-center justify-between mb-8 px-2 md:px-4">
           <h2 className={`text-lg md:text-xl font-black ${isDark ? 'text-white' : 'text-slate-900'} flex items-center w-full`}>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 md:gap-6">
                ...
                >
                  ...
                  <div className="flex justify-between items-start mb-6 relative z-10">
                    <div className="space-y-2 md:space-y-3">
                       ...
                       <h3 className={`font-black text-lg md:text-xl ${isDark ? 'text-white' : 'text-slate-900'} leading-tight group-hover:text-blue-500 transition-colors truncate max-w-[150px] md:max-w-none`}>
                         {lead.applicant_name}
                       </h3>
                    </div>
                    <div className={`px-2.5 py-1 md:px-3 md:py-1.5 rounded-full text-[8px] md:text-[9px] font-black uppercase tracking-widest border shadow-sm ${getStatusColor(lead.status)}`}>
                      {lead.status}
                    </div>
                  </div>

                  <div className={`grid grid-cols-2 gap-4 md:gap-6 pt-5 md:pt-6 border-t ${isDark ? 'border-white/5' : 'border-slate-100'} relative z-10`}>
                    <div>
            ...
            {filteredLeads.length === 0 && !loading && (
              <div className={`col-span-full py-20 md:py-32 text-center flex flex-col items-center justify-center ${isDark ? 'bg-white/2 border-white/5' : 'bg-slate-100/50 border-slate-200'} rounded-[32px] md:rounded-[40px] border border-dashed`}>
                ...
                <h3 className={`text-xl md:text-2xl font-black ${isDark ? 'text-white' : 'text-slate-900'} mb-2`}>No matching entries</h3>
...
const StatCard = ({ title, value, icon, color, delay, isDark }) => (
  <motion.div 
    ...
    transition={{ delay }}
    className={`${isDark ? 'bg-white/5 border-white/5 shadow-22xl shadow-black/40' : 'bg-white border-slate-200 shadow-xl shadow-slate-200/30'} p-4 md:p-8 rounded-2xl md:rounded-[40px] border relative overflow-hidden group active:scale-95 transition-all`}
  >
    ...
    <div className="relative z-10 text-center md:text-left">
      ...
      <h3 className={`text-lg sm:text-2xl md:text-4xl font-black ${isDark ? 'text-white' : 'text-slate-900'} tracking-tighter leading-none`}>{value}</h3>
    </div>
  </motion.div>
);
