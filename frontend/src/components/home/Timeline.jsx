...
const Timeline = () => {
  ...
  return (
    <section className={`py-24 ${isDark ? 'bg-[#0B0F19]' : 'bg-white'} relative overflow-hidden font-dmsans transition-colors duration-300`}>
      <div className="max-w-7xl mx-auto px-4 relative z-10">
        <div className="text-center mb-20">
          <motion.div 
            initial={{ opacity: 0, y: 10 }}
            whileInView={{ opacity: 1, y: 0 }}
            className={`inline-flex items-center gap-2 px-4 py-1.5 rounded-full ${isDark ? 'bg-white/5 border-white/5' : 'bg-slate-100 border-slate-200'} text-primary-gold text-[10px] font-black uppercase tracking-[0.3em] mb-4 border shadow-2xl`}
          >
          ...
          {steps.map((step, idx) => (
            ...
            >
              <div className="relative mb-10 inline-block">
                <div 
                  className={`w-24 h-24 ${isDark ? 'bg-white/5 border-white/10' : 'bg-slate-50 border-slate-200'} backdrop-blur-sm border rounded-[36px] flex items-center justify-center text-primary-gold group-hover:bg-primary-gold group-hover:text-[#020617] transition-all duration-700 shadow-2xl group-hover:shadow-primary-gold/20`}
                >
              ...
              <h4 className={`${isDark ? 'text-white' : 'text-slate-900'} font-playfair font-black text-xl mb-3 group-hover:text-primary-gold transition-colors`}>{step.title}</h4>
              <p className={`text-slate-500 text-[10px] uppercase font-bold tracking-tight leading-relaxed px-6 ${isDark ? 'group-hover:text-slate-300' : 'group-hover:text-slate-700'} transition-colors`}>
                {step.desc}
              </p>
