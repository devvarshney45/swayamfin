  ...
  return (
    <section className={`py-24 ${isDark ? 'bg-[#020617]' : 'bg-[#F8FAFC]'} relative overflow-hidden font-dmsans transition-colors duration-300`}>
      <div className="max-w-7xl mx-auto px-4 text-center mb-16">
        <h2 className={`text-3xl font-playfair font-black ${isDark ? 'text-white' : 'text-slate-900'} mb-4`}>Strategic NBFC Partnerships</h2>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-8 md:gap-12 items-center px-4 max-w-7xl mx-auto">
         {[
           ...
           <motion.div
             ...
             className={`flex items-center justify-center p-6 ${isDark ? 'bg-white/5 hover:border-white/10 text-slate-500 hover:text-white' : 'bg-slate-100/50 hover:border-slate-300 text-slate-500 hover:text-primary-gold'} rounded-3xl border border-transparent font-playfair font-black text-center text-sm transition-all duration-300 cursor-default opacity-50 hover:opacity-100 shadow-sm hover:shadow-lg`}
           >
             {partner}
           </motion.div>
         ))}
